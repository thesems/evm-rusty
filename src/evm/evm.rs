    ...
    fn process_operation(&mut self, operation: &Operation) -> Result<ExecutionResult, VMError> {
        ...
        match operation {
            ...
            Operation::Keccak256 => {
                ...
                let dynamic_gas = 6 * minimum_word_size + Memory::calc_memory_expansion_gas(size);

                if self.gas_available < gas_cost.base + dynamic_gas {
                    return Err(VMError::OutOfGas);
                }

                self.memory
                    .expand_memory(offset, size, self.gas_available)?;

                let hash = keccak256(self.memory.read(offset, size));
                self.stack.push(U256::from_be_slice(hash.as_slice()))?;

                return Ok(ExecutionResult::Success {
                    return_data: None,
                    gas_used: gas_cost.base + dynamic_gas,
                    jump_dest: 0,
                    halt: false,
                    new_contract_address: None,
                });
